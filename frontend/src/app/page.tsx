      ...
      <header className="border-b border-[#1a3d22] bg-[#050f08]/80 backdrop-blur-sm sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <a
              href="/picks"
              className="text-xs font-mono text-[#4a6b50] hover:text-[#00ff6a] transition-colors border border-[#1a3d22] px-3 py-1 rounded"
            >
              📋 Daily Picks
            </a>
            {user ? (
              <div className="flex items-center gap-2">
                <span className="text-xs font-mono text-[#4a6b50] hidden sm:block">{user.role.toUpperCase()}</span>
                <button
                  onClick={signOut}
                  className="text-xs font-mono text-[#1a3d22] hover:text-red-400 transition-colors"
                >
                  Ieși
                </button>
              </div>
        ...
        {prediction && (
          <div className="space-y-4 animate-in">
            ...
            <div className="ticket-card p-6">
              ...
              {/* Betting Signals */}
              {prediction.betting_signals && prediction.betting_signals.confidence_label && (
                <div className="mt-5 grid grid-cols-2 md:grid-cols-4 gap-3">
                  <div className={`border rounded-lg px-3 py-3 text-center ${
                    prediction.betting_signals.confidence_label === 'HIGH'
                      ? 'border-[#00ff6a]/30 bg-[#00ff6a]/5'
                      : 'border-yellow-800/30 bg-yellow-900/5'
                  }`}>
                    <p className="text-[10px] font-mono text-[#4a6b50] uppercase tracking-widest">Confidence</p>
                    <p className={`font-display font-700 text-xl ${
                      prediction.betting_signals.confidence_label === 'HIGH' ? 'text-[#00ff6a]' : 'text-yellow-400'
                    }`}>
                      {prediction.betting_signals.confidence}%
                    </p>
                    <p className={`text-[10px] font-mono uppercase ${
                      prediction.betting_signals.confidence_label === 'HIGH' ? 'text-[#00ff6a]/60' : 'text-yellow-400/60'
                    }`}>
                      {prediction.betting_signals.confidence_label}
                    </p>
                  </div>

                  <div className="border border-[#1a3d22] rounded-lg px-3 py-3 text-center">
                    ...
                    <p className="font-display font-700 text-xl text-white">
                      {prediction.betting_signals.kelly_pct}%
                    </p>
                  ...
                  <div className="border border-[#1a3d22] rounded-lg px-3 py-3 text-center">
                    ...
                    <p className="font-display font-700 text-xl text-[#4a6b50]">
                      {prediction.betting_signals.edge_pct > 0 ? `+${prediction.betting_signals.edge_pct}%` : '—'}
                    </p>
                    <p className="text-[10px] font-mono text-[#1a3d22]">vs Pinnacle</p>
                  </div>

                  <div className={`border rounded-lg px-3 py-3 text-center ${
                    prediction.betting_signals.value_bet
                      ? 'border-[#00ff6a]/30 bg-[#00ff6a]/5'
                      : 'border-[#1a3d22]'
                  }`}>
                    <p className="text-[10px] font-mono text-[#4a6b50] uppercase tracking-widest">Value Bet</p>
                    <p className="text-2xl mt-1">{prediction.betting_signals.value_bet ? '💎' : '—'}</p>
                    <p className={`text-[10px] font-mono uppercase ${
                      prediction.betting_signals.value_bet ? 'text-[#00ff6a]/60' : 'text-[#1a3d22]'
                    }`}>
                      {prediction.betting_signals.value_bet ? 'ACTIV' : 'INACTIV'}
                    </p>
                  </div>
                </div>
              )}
