...
setFixtures(filtered.length > 0 ? filtered : upcoming.length > 0 ? upcoming : all.slice(0, 30))
  ...
  return (
    <div className="app-bg grid-bg min-h-screen flex flex-col overflow-x-hidden">
      {/* Header */}
      <header className="header sticky top-0 z-50">
        <div className="max-w-6xl mx-auto px-4 py-2 flex items-center justify-between">
          ...
          <nav className="flex items-center gap-1">
            ...
            <button onClick={() => window.location.href='/weekly/index.html'} className="nav-link">{tr.results}</button>
          </nav>
        </div>
      </header>
