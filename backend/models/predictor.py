...
def _compute_betting_signals(home_win: float, draw: float, away_win: float) -> dict:
    """
    Calculează semnale de betting din probabilitățile finale ale modelului.
    home_win, draw, away_win sunt procente (0-100).

    Returnează:
      - top_outcome: "home_win" | "draw" | "away_win"
      - confidence: float (0-100)
      - confidence_label: "HIGH" | "MEDIUM" | None
      - kelly_pct: float (max 10%, fractional 1/4 Kelly)
      - edge_pct: float (0.0 până la integrare Pinnacle)
      - value_bet: bool (confidence ≥65% AND edge ≥5%)
    """
    outcomes = [
        ("home_win", home_win),
        ("draw", draw),
    ...
    top_outcome, confidence = max(outcomes, key=lambda x: x[1])
    ...
    # Kelly fracțional 1/4 — fără Pinnacle folosim fair odds cu margine 5%
    p = confidence / 100
    if p > 0:
        decimal_odds = (1 / p) * 0.95  # piața ia ~5% margine
        b = decimal_odds - 1
        kelly_raw = (b * p - (1 - p)) / b if b > 0 else 0
        kelly_pct = round(min(max(kelly_raw * 25, 0), 10.0), 2)  # 1/4 Kelly, max 10%
    else:
        kelly_pct = 0.0

    # Edge % vs Pinnacle — placeholder până la integrare reală
    edge_pct = 0.0

    # Value Bet: necesită ambele condiții
    value_bet = confidence >= 65 and edge_pct >= 5.0
...
class FootballPredictor:
    ...
    async def predict(
        ...
        signals = _compute_betting_signals(final_home, final_draw, final_away)
    ...
    def demo_prediction(self, home_team: str, away_team: str) -> dict:
        ...
        demo_signals = _compute_betting_signals(52.4, 24.1, 23.5)
        return {
            "match": f"{home_team} vs {away_team}",
            "home_team": home_team,
            "away_team": away_team,
